...
<Note>
  **Agents are currently in an experimental phase** and not yet available to all
  users. For now, Kody operates through the **GenericAgent**, which has access
  to all tools described below. This allows Kody to perform tasks similar to
  each specialized agent, such as conducting a code review or generating
  documentation, by leveraging the power of the available tools.
</Note>
...
### CodeReviewer (Coming Soon)

Gives in-depth feedback on code quality, style, potential security risks, and other issues to keep your codebase clean, secure, and scalable. CodeReviewer highlights improvements just like a senior dev reviewing your PRs.

### DocHelper (Coming Soon)
...
Currently, Kody operates primarily through the **GenericAgent**, which has access to all tools described below. While the specialized agents are in development, the GenericAgent can still handle a wide variety of tasks by using the available tools, achieving results similar to the planned agents. For example, you can request a code review, and the GenericAgent will analyze code quality, identify potential issues, and provide feedback based on best practices.
...
Performs a structured review on code quality, security, efficiency, and best practices, making it easy to keep standards high.
...
Tracks team performance metrics like Throughput, Lead Time, and **DORA Metrics** (e.g., Deployment Frequency, Change Failure Rate), providing real-time and historical comparisons for insights on productivity trends.
