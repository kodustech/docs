...
Thinking トグルで **Custom** を選択すると JSON テキストエリアが表示されます。Kodus がモデルを呼び出すときに Vercel AI SDK の `providerOptions` にマージする生のオブジェクトを貼り付けます。以下の場合に使用します：

- Claude の特定の `budget_tokens` 値が必要な場合（プリセットの effort マッピングの代わり）
- OpenAI 互換プロバイダーでモデルごとに thinking を有効/無効にしたい場合
- Kodus がまだマッピングしていない新しい reasoning フィールドがプロバイダーに追加された場合
- プリセットがカバーしない reasoning フィールドを設定したい場合（例：`reasoningSummary`）

#### 名前空間ルール

Vercel AI SDK は**名前空間キー**を通じてプロバイダー固有のオプションをルーティングします。JSON は BYOK 設定の**アクティブなプロバイダー**に一致する名前空間の下でフィールドをラップする必要があります：

| BYOK プロバイダー | JSON の名前空間キー |
| --- | --- |
| `anthropic` | `anthropic` |
...
誤った名前空間を使用すると、SDK はサイレントにオプションを破棄します — 呼び出しは成功しますが、オーバーライドは無視されます。
...
<Tabs>
  <Tab title="Anthropic">
    ...
    `high` effort で adaptive thinking を強制（"High" プリセットを選択したときと同じ — 参考のために示します）：
  ...
  <Tab title="Google Gemini">
    明示的な thinking 予算を設定（Gemini 2.5 ファミリーは予算を使用、Gemini 3+ はレベルを使用）：
  ...
  <Tab title="OpenAI">
    カスタムサマリーモードで reasoning effort を設定：
  ...
  <Tab title="OpenRouter">
    OpenRouter のルーティング層を通じて reasoning effort を強制：
  ...
  <Tab title="OpenAI-compatible (Kimi、GLM など)">
    Kimi/GLM の予算ヒント付きで thinking を有効化：
    ...
    thinking を明示的に無効化（これらのモデルではデフォルトで有効）：
...
- **有効な JSON のみ。** カンマの欠落や末尾のカンマはパースを壊し、Kodus はオーバーライドを無視します。フォームは保存前にバリデーションエラーを表示します。
- **間違った名前空間 = サイレント no-op。** プロバイダーキーの下にラップせずに `{ "thinking": {...} }` を直接貼り付けても何もしません。
- **優先順位:** JSON オーバーライドはプリセットが生成するものを**完全に置き換えます** — オーバーライドしてもフィールドが欠けている場合、それは送信されません。
- **新しいプロバイダーフィールド:** Anthropic / Google / OpenAI がプリセットマッパーを更新する前に新しい reasoning パラメータをリリースした場合、JSON オーバーライドが脱出ハッチです。
