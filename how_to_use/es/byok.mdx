...
Elegir **Custom** en el interruptor de Thinking revela un área de texto JSON. Pega un objeto en crudo que Kodus fusiona con `providerOptions` del Vercel AI SDK cuando llama al modelo. Úsalo cuando:

- Necesitas un valor específico de `budget_tokens` para Claude (en lugar del mapeo predefinido de esfuerzo)
- Quieres habilitar/deshabilitar el razonamiento por modelo en proveedores compatibles con OpenAI
- El proveedor lanza un nuevo campo de razonamiento que Kodus aún no ha mapeado
- Quieres establecer campos de razonamiento que los presets no cubren (por ejemplo, `reasoningSummary`)

#### La regla del namespace

El Vercel AI SDK enruta las opciones específicas del proveedor mediante **claves de namespace**. Tu JSON debe envolver los campos bajo el namespace correspondiente al **proveedor activo** en la configuración de BYOK:

| Proveedor de BYOK | Clave de namespace en tu JSON |
| --- | --- |
| `anthropic` | `anthropic` |
...
Si usas el namespace incorrecto, el SDK descarta silenciosamente las opciones — la llamada tiene éxito pero tu sobrescritura se ignora.
...
<Tabs>
  <Tab title="Anthropic">
    ...
    Forzar pensamiento adaptativo con esfuerzo `high` (igual que elegir el preset "High" — mostrado como referencia):
  ...
  <Tab title="Google Gemini">
    Establecer un presupuesto de pensamiento explícito (las familias de Gemini 2.5 usan presupuestos, Gemini 3+ usa niveles):
    ...
    Forzar el nivel de pensamiento:
  ...
  <Tab title="OpenAI">
    Establecer el esfuerzo de razonamiento con un modo de resumen personalizado:
  ...
  <Tab title="OpenRouter">
    Forzar el esfuerzo de razonamiento a través de la capa de enrutamiento de OpenRouter:
  ...
  <Tab title="Compatible con OpenAI (Kimi, GLM, etc.)">
    Habilitar pensamiento con una sugerencia de presupuesto de Kimi/GLM:
    ...
    Deshabilitar explícitamente el pensamiento (está habilitado por defecto en estos modelos):
...
- **Solo JSON válido.** Las comas faltantes o sobrantes rompen el análisis y Kodus ignora la sobrescritura. El formulario muestra un error de validación antes de guardar.
- **Namespace incorrecto = no-op silencioso.** Pegar `{ "thinking": {...} }` directamente (sin envolverlo bajo una clave de proveedor) no hace nada.
- **Precedencia:** la sobrescritura JSON **reemplaza por completo** lo que emitiría el preset — si sobrescribes pero omites un campo, simplemente no se envía.
- **Nuevos campos del proveedor:** cuando Anthropic / Google / OpenAI lanzan un nuevo parámetro de razonamiento antes de que actualicemos el mapeador de presets, la sobrescritura JSON es la salida de escape.
