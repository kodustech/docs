...
当你的组织已经配置了规则后,Review Rules 标签页提供了查找、组织和批量操作规则的工具。

#### 搜索 & 筛选

使用列表顶部的搜索栏来匹配规则的**标题、路径、说明和示例片段**(在页面任意位置按 `/` 可聚焦搜索框)。

**Filters** 弹出窗口用于细化可见列表:

- **Sort by** — Recently updated、Severity (high → low) 或 A → Z。
- **View** — 切换显示哪些范围:当前范围内的规则、从其他目录继承的规则、从仓库继承的规则、从全局继承的规则,以及已禁用的规则。
- **Origin** — 限制规则来源:Auto-sync(从 `.cursor/rules`、`.codex/rules` 等导入)、Onboard、Kody-generated、Library 或 Manual。
- **Status** — `Has sync errors`(IDE 同步失败的规则)和 `Paused only`(参见下方[暂停规则](#暂停规则-vs-删除))。

<Frame caption="Filters 弹出窗口包含 Sort by、View、Origin 和 Status 区域">
  <img src="/images/kody-rules-filters-popover.png" />
</Frame>

列表上方的严重性热力图(例如 `2 Critical`、`5 High`)显示当前可见规则的分布,同时也作为一键筛选器 — 点击芯片可将列表缩小到该严重性,再次点击可清除。

<Frame caption="列表上方的严重性芯片同时充当一键筛选器">
  <img src="/images/kody-rules-severity-heatmap.png" />
</Frame>

启用的筛选器以可移除的芯片形式显示在列表上方;点击 `Clear all` 可一次性重置所有筛选。

#### 暂停规则 vs 删除

每条规则都有一个状态,用于控制它是否在代码审查中执行:

| 状态 | 效果 | 何时使用 |
|------|------|----------|
| **Active** | 规则在每个适用的 PR 上运行。 | 想要执行的规则的默认状态。 |
| **Paused** | 规则保留在列表中,但在审查中被跳过。 | 临时静默一个噪声较大的规则、在重构期间临时禁用、A/B 测试改动 — 同时不丢失规则定义。 |
| **Deleted** | 规则被移除,不再适用。 | 永久;规则不再出现在列表中。 |

暂停是可逆的 — 已暂停的规则保留完整定义和历史记录,恢复后会立即重新运行。删除是永久性的。

#### 批量操作

对于拥有数十或数百条规则的仓库,批量操作允许一次性操作多条规则。

1. **勾选**任意规则卡片上的复选框开始一次选择。列表顶部会出现一个工具栏,显示数量和可用操作。
2. 你可以从那里:
   - **Select all visible (N)** — 将选择扩展到当前视图中的每一条符合条件的规则(筛选和搜索会被考虑,因此可以先缩小再批量选择)。
   - **Clear** — 重置选择。
   - **Pause N** — 暂停所有当前为 Active 的所选规则。仅当所选规则中至少有一条可暂停时才出现。
   - **Resume N** — 将所有已暂停的所选规则恢复为 Active。仅当所选规则中至少有一条已暂停时才出现。
   - **Delete N** — 永久删除所选项。一个确认弹窗会列出即将移除的规则标题;此操作无法撤销。

<Frame caption="选择一条或多条规则后会出现批量操作工具栏">
  <img src="/images/kody-rules-bulk-toolbar.png" />
</Frame>

<Frame caption="批量删除确认弹窗在执行破坏性操作前列出所选标题">
  <img src="/images/kody-rules-bulk-delete-modal.png" />
</Frame>

<Info>
  继承的规则(带有 `INHERITED` 徽章、来自父级范围的规则)无法从子级范围进行批量选择 — 要删除或暂停它们,请前往最初定义它们的范围。
</Info>
