...
Picking **Custom** in the Thinking toggle reveals a JSON textarea. Paste a raw object that Kodus merges into the Vercel AI SDK `providerOptions` when it calls the model. Use this when:

- You need a specific `budget_tokens` value for Claude (instead of the preset effort mapping)
- You want to enable/disable thinking on a per-model basis for OpenAI-compatible providers
- The provider ships a new reasoning field Kodus hasn't mapped yet
- You want to set reasoning fields the presets don't cover (e.g. `reasoningSummary`)

#### The namespace rule

Vercel AI SDK routes provider-specific options via **namespace keys**. Your JSON must wrap fields under the namespace matching the **active provider** in the BYOK config:

| BYOK provider | Namespace key in your JSON |
| --- | --- |
| `anthropic` | `anthropic` |
...
If you use the wrong namespace, the SDK silently drops the options — the call succeeds but your override is ignored.
...
<Tabs>
  <Tab title="Anthropic">
    ...
    Force adaptive thinking with `high` effort (same as picking "High" preset — shown for reference):
  ...
  <Tab title="Google Gemini">
    Set an explicit thinking budget (Gemini 2.5 families use budgets, Gemini 3+ uses levels):
  ...
  <Tab title="OpenAI">
    Set reasoning effort with a custom summary mode:
  ...
  <Tab title="OpenRouter">
    Force reasoning effort through the OpenRouter routing layer:
  ...
  <Tab title="OpenAI-compatible (Kimi, GLM, etc.)">
    Enable thinking with a Kimi/GLM budget hint:
    ...
    Explicitly disable thinking (defaults to enabled on these models):
...
- **Valid JSON only.** Missing commas or trailing commas break the parse and Kodus ignores the override. The form shows a validation error before save.
- **Wrong namespace = silent no-op.** Pasting `{ "thinking": {...} }` directly (without wrapping under a provider key) does nothing.
- **Precedence:** the JSON override **fully replaces** what the preset would emit — if you override but miss a field, it's just not sent.
- **New provider fields:** when Anthropic / Google / OpenAI ship a new reasoning parameter before we update the preset mapper, the JSON override is the escape hatch.
